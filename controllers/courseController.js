...
router.get('/', (req, res) => {
    res.render("course/courseAddEdit", {viewTitle: "Course Registration Chrysna Tech"});
});
    ...
    Course.findOneAndUpdate({
        ...
        if (!err) {
            ...
            if (err.name == 'ValidationError') {
                ...
                res.render("course/courseAddEdit", { // Retaining value to be displayed in the child view
...
router.get('/list', (req, res) => {
    Course.find((err, docs) => {
        if (!err) {
            res.render("course/list", {list: docs});
        } else {
...
function handleValidationError(err, body) {
    for (field in err.errors) {
        switch (err.errors[field].path) {
            case 'courseName': body['courseNameError'] = err.errors[field].message;
                break;
            default:
                break;
        }
